estimateRMSE <- function(E_centered, N, treatment_group, control_group, idx_training, idx_val) {
    ## Description: estimates the differential activities of TE subfamilies by multiple regression,
    # and uses the activities estimated on the training observations to estimate the root mean squared error (RMSE) on the test observations

        ## Para: -E centered: matrix of normalized gene expression values with genes as rows and samples as columns
                #-N: TE cis-regulatory predictor matrix with genes as rows and TE subfamilies as columns
                #-treatment_group: vector with the name of treated samples (must correspond to columns in E_centered)
                #-control_group: vector with the name of control samples (must correspond to columns in E_centered)
                #-idx_training: indexes the training observations
                #-idx_val: indexes the validation observations
